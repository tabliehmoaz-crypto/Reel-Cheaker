/*
  REELIQ — ACCOUNT MEMORY V2
  --------------------------------
  ذاكرة الحساب المركزية.
  ...
  - تجهيز البيانات للـLearning Engine

  Local-first الآن.
  Cloud/Google Sync لاحقاً بدون تغيير الـAPI الأساسي.
*/


const STORAGE_KEY = "reeliq_account_memory_v2";
...
export function saveExperiment(
  ...
  if (
    ...
    throw new Error(
      "REELIQ Memory: invalid experiment."
    );
...
export function getLearnings() {
  ...
  for (
    const experiment of state.experiments
  ) {
...
function updateExperiment(
  ...
  if (
    ...
    throw new Error(
      `REELIQ Memory: Experiment not found: ${id}`
    );
  ...
  if (
    ...
    throw new Error(
      "REELIQ Memory: updater must return experiment."
    );
