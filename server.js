...
/*
  تقديم index.html الجديد (المحرك المحلي) كصفحة رئيسية،
  بالإضافة لكل ملفات src/ حتى تشتغل استدعاءات import
  الخاصة بالمتصفح (ES Modules) بشكل صحيح.

  الأداة القديمة (تحليل عبر Gemini API السحابي) صارت
  على /legacy-cloud-analyzer بدل ما تكون الصفحة الرئيسية،
  لأنها مسار منفصل تماماً عن محرك التحليل المحلي.
*/
...
app.get('/legacy-cloud-analyzer', (req, res) => {
  res.send(UI_HTML);
});
